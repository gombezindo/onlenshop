...
class Main extends BASE_Controller {
	...
	public function index_old($page = 1)
	{
	...
	{
		$limit = 6;
		$latest = $this->produk_model->get_latest_produk_home($limit);
		$populer = $this->produk_model->get_populer_produk_home($limit); 

		$data_view = array('latest' => $latest,
			'populer' => $populer);

		$grid = $this->load->view('frontend/main', $data_view, TRUE);
		$this->tampilMain($grid);
	}
